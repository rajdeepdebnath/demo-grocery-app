...
const CatalogItem = ({ item }: Props) => {
  const wishlistIds = useAppSelector(
    (state: RootState) => state.wishlist.wishlistProductIds
  );
        ...
        <Stack direction="column" sx={{ width: "100%" }}>
          <Box sx={itemStyle}>
            ...
            <Stack direction="row">
              {item.available < 10 ? (
                <Chip
                  label={`Only ${item.available} left`}
                  sx={{ backgroundColor: "#f9c056", width: "40%" }}
                  size="small"
                />
              ) : (
                <Chip
                  label="Available"
                  sx={{ backgroundColor: "#6dbd75", width: "40%" }}
                  size="small"
                />
              )}
              {Boolean(coupon) && (
          ...
          <Grid
            ...
            spacing={1}
            sx={{
              backgroundColor: { xs: "#f7f5f5", md: "#fff" },
              width: "100% !important",
          ...
          >
            ...
            >
              {isItemInCart() && (
                <CartCounter item={item} quantity={getCartItemQuantity()} />
              )}
            </Grid>
